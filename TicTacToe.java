...
public class TicTacToe
{
    ...
    { 
        for (int row = 0; row < 3; row++)
        {
            for (int col = 0; col < 3; col++)
            {
                if (board[0][0] != PLACEHOLDER && board[0][0] == board[1][1] && board[0][0] == board[2][2])
                {
                    return true;
                }
                if (board[0][2] != PLACEHOLDER && board[0][2] == board[1][1] && board[0][2] == board[2][0])
                {
                    return true;
                }
            }
    ...
    public static void getPositionAndPlaceToken(char[][] board, boolean isXTurn)
    {
        ...
        do {
            if (isXTurn)
            {
                System.out.print("X pick your square:  ");
            }
            else {
                System.out.print("O pick your square:  ");
            }
            position = keyboard.nextInt();
            invalidInput = (position < 1 || position > 9);
            if (invalidInput){
                System.out.println("That input was invalid.   Please enter a 1-9");
            } else {

                full = (getTokenAtPosition(position, board) != PLACEHOLDER);
                if (full){
                    System.out.println("That position is full, please try again.");
                }
            }
